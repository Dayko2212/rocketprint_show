...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-[80vh] relative bg-white/60 dark:bg-transparent overflow-hidden">
      {/* Background Image Overlay with deep blue base */}
      <div
        className="absolute inset-0 z-0 opacity-40 dark:opacity-40 light:opacity-5 mix-blend-screen bg-repeat pattern-overlay pointer-events-none"
        style={{ backgroundImage: "url('/galaxy_bg.png')", backgroundSize: "400px" }}
      />

      {/* Subtle radial gradient only in Dark mode */}
      ...
      <section className="relative z-10 text-foreground min-h-[60vh] flex flex-col items-center justify-center py-16">
        <div className="container mx-auto px-4 text-center">
          <div className="inline-block p-1 bg-gradient-to-r from-blue-500/20 to-orange-500/20 rounded-full mb-6">
            <span className="px-5 py-2 rounded-full bg-white/80 dark:bg-card-bg backdrop-blur-md border border-card-border text-orange-400 font-bold tracking-widest text-sm uppercase">💡 Innovation & Apprentissage</span>
          </div>

          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-8 leading-tight drop-shadow-2xl">
            L'Ingénierie sans limite <br className="hidden md:block" />
            pour les <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500 font-black">Inventeurs Passionnés</span>
          </h1>

          <p className="text-xl md:text-2xl text-foreground/80 dark:text-foreground/70 mb-12 max-w-3xl mx-auto font-medium leading-relaxed drop-shadow-lg">
            Découvrez notre dernier bras robotique de précision conçu par notre équipe pour stimuler l'innovation. Une expérience d'ingénierie captivante pour les passionnés.
          </p>

          <div className="flex flex-col sm:flex-row justify-center gap-6">
            <Link
              href={`/products/${product.slug}`}
              className="bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-400 hover:to-red-500 text-white px-10 py-5 rounded-2xl font-black text-xl transition-all shadow-[0_0_40px_-10px_rgba(249,115,22,0.6)] transform hover:-translate-y-1 hover:scale-105"
            >
              ⚙️ Explorer le modèle en 3D
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="relative z-10 py-16 bg-gradient-to-b from-transparent to-background flex-grow">
